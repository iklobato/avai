...
class _FakeClient:
    ...
    def __init__(self, parsed=None, parsed_per_call=None,
...
class TestHappyPath:
    def test_returns_judgments_for_each_index(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": 0, "verdict": "malicious",
             "category": "persistence", "confidence": 0.9,
             "reasoning": "bad", "remediation": "remove it"},
            {"index": 1, "verdict": "benign",
             "category": "none", "confidence": 0.99,
             "reasoning": "ok", "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        result = judge.judge("processes", "h",
                             [{"content_hash": "a", "name": "x"},
                              {"content_hash": "b", "name": "y"}])
        assert len(result) == 2
        assert {j.verdict for j in result} == {Verdict.MALICIOUS,
                                               Verdict.BENIGN}
    ...
    def test_user_template_is_substituted(self, prompts):
        client = _FakeClient()
        judge = _judge(prompts, client)
        judge.judge("processes", "be careful",
                    [{"content_hash": "a", "name": "x"}])
...
class TestParsingDefensiveness:
    ...
    def test_judgment_index_out_of_range_is_skipped(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": 5, "verdict": "benign", "category": "none",
             "confidence": 1.0, "reasoning": "x", "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        # Only one entry → index 5 is invalid; nothing produced.
        result = judge.judge("processes", "",
                             [{"content_hash": "a"}])
        assert result == []

    def test_judgment_index_negative_is_skipped(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": -1, "verdict": "benign", "category": "none",
             "confidence": 1.0, "reasoning": "x", "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        assert judge.judge("processes", "",
    ...
    def test_judgment_with_non_integer_index_is_skipped(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": "0", "verdict": "benign", "category": "none",
             "confidence": 1.0, "reasoning": "x", "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        assert judge.judge("processes", "",
    ...
    def test_invalid_verdict_value_falls_back_to_unknown(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": 0, "verdict": "spicy", "category": "none",
             "confidence": 0.5, "reasoning": "?", "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        result = judge.judge("processes", "",
                             [{"content_hash": "a"}])
    ...
    def test_invalid_category_value_falls_back_to_none(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": 0, "verdict": "benign", "category": "fake-cat",
    ...
    def test_non_numeric_confidence_defaults_to_zero(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": 0, "verdict": "benign", "category": "none",
             "confidence": "high", "reasoning": "ok", "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        result = judge.judge("processes", "",
                             [{"content_hash": "a"}])
    ...
    def test_confidence_is_clamped_to_zero_one(self, prompts):
        client = _FakeClient(parsed={"judgments": [
            {"index": 0, "verdict": "benign", "category": "none",
             "confidence": 1.5, "reasoning": "ok", "remediation": ""},
            {"index": 1, "verdict": "benign", "category": "none",
             "confidence": -0.5, "reasoning": "ok", "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        result = judge.judge("processes", "",
                             [{"content_hash": "a"},
    ...
    def test_reasoning_is_truncated_to_500_chars(self, prompts):
        ...
        client = _FakeClient(parsed={"judgments": [
            {"index": 0, "verdict": "benign", "category": "none",
             "confidence": 1.0, "reasoning": long, "remediation": ""},
        ]})
        judge = _judge(prompts, client)
        result = judge.judge("processes", "",
                             [{"content_hash": "a"}])
...
class TestBatching:
    def test_splits_into_batches_of_batch_size(self, prompts):
        client = _FakeClient(parsed={"judgments": []})
        judge = LlmJudge(prompts=prompts, model="m",
                         batch_size=3, max_per_collector=0,
                         client=client)
        entries = [{"content_hash": f"h{i}", "name": str(i)}
                   for i in range(10)]
    ...
    def test_caps_entries_to_max_per_collector(self, prompts):
        client = _FakeClient(parsed={"judgments": []})
        judge = LlmJudge(prompts=prompts, model="m",
                         batch_size=20, max_per_collector=3,
                         client=client)
...
class TestFailureIsolation:
    def test_one_failing_batch_does_not_abort_later_batches(self, prompts):
        ...
        results_per_call = [Exception("boom"),
                            {"judgments": [
                                {"index": 0, "verdict": "benign",
        ...
        judge = LlmJudge(prompts=prompts, model="m",
                         batch_size=1, max_per_collector=0,
                         client=client)
